...
const favouriteProblem = async (req, res) => {
    try {
        ...
        // checking if the problem is already favourited by the user
        const alreadyFavourite = user.favouriteProblems?.includes(problemId);
        if (alreadyFavourite) {
            return res.status(200).send({
                message: "Problem already in favourite's list",
                favourite: true,
            });
        }

        // adding problem in user's favourite problem's list
        user.favouriteProblems = user.favouriteProblems || [];
        ...
        return res.status(200).json({
            message: "Problem added successfully in favourite's list",
            favourite: true
        });
...
module.exports = favouriteProblem;